переменные объявляются с использованием ключевого слова `var` или с помощью короткого объявления `:=`. Вот основные моменты, которые нужно знать о переменных в Go:
- Пример 1
